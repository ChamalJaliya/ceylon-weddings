> **Superseded.** This file has been replaced by [MODULES.md](./MODULES.md), which reflects the current implementation status (MVP ✅ / v1.5 🔶 / v2 🔮). The research content below is preserved for historical context.
...
**Shipping shape:** one public website (`apps/web`) with role-based surfaces, not three Next apps. The module list below is unchanged.
...
- Admin: vendor verification, abuse handling, featured placement inventory, Ideas CMS, support lookup, audit trail (`/admin/*`, see `specs/002-admin.md`)
